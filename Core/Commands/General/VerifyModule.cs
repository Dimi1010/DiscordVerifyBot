    ...
    public partial class VerifyModule : ModuleBase<SocketCommandContext>
    {
        ...
        public async Task AddL1User(IGuildUser user)
        {
            ...
            await _replyservice.ReplyEmbedAsync(context: Context,
                message: $"User {user.Mention} has been granted verifier permissions.");
        }
        ...
        public async Task RemoveL1User(IGuildUser user)
        {
            ...
            {
                await _replyservice.ReplyEmbedAsync(context: Context,
                    message: $"User {user.Mention} does not have verifier permissions.");
            }
            else
            {
                ...
                await _replyservice.ReplyEmbedAsync(context: Context,
                    message: $"User {user.Mention}'s verifier permissions have been revoked.");
            }
        }
        ...
        public async Task AddL2User(IGuildUser user)
        {
            ...
            await _replyservice.ReplyEmbedAsync(context: Context,
                message: $"User {user.Mention} has been granted approver permissions.");
        }
        ...
        public async Task RemoveL2User(IGuildUser user)
        {
            ...
            {
                await _replyservice.ReplyEmbedAsync(context: Context,
                    message: $"User {user.Mention} does not have approver permissions.");
            }
            else
            {
                await DiscordUserDataHandler.RemoveGuildUser(res);
                ...
                await _replyservice.ReplyEmbedAsync(context: Context,
                    message: $"User {user.Mention}'s approver permissions have been revoked.");
            }
        }
        ...
        public async Task Verify(IGuildUser user)
        {
            ...
            if (invokingUser.PermissionLevel == DiscordGuildUser.PermissionLevels.Approve)
            {
                ...
                var message = $"User {user.Mention} has been verified ";
                if(form.Verifier != Context.User.Id)
                    message += $"by { Context.Guild.GetUser(form.Verifier).Mention } ";
                ...
                {
                    await _replyservice.ReplyEmbedAsync(context: Context,
                        message: $"User {user.Mention} already has a pending verification form");
                }
            }
        }
        ...
        public async Task GetPendingForGuild()
        {
            ...
            foreach(var form in query)
            {
                ...
                {
                    Name = Context.Guild.GetUser(form.Verified).Mention,
                    Value = $"Verifier: { Context.Guild.GetUser(form.Verifier).Mention } \n Issued: { form.IssuedUtc.ToString(format: "dd/MM/yyyy HH:mm") }"
                };
        ...
        public async Task ApproveUser(IGuildUser user)
        {
            ...
            {
                await _replyservice.ReplyEmbedAsync(context: Context,
                    message: $"No pending verification for user { user.Mention }");
            ...
            await _replyservice.ReplyEmbedAsync(context: Context,
                message: $"User {user.Mention} has been approved.");
        ...
        public async Task ApproveAll()
        {
            ...
            await _replyservice.ReplyEmbedAsync(context: Context,
                message: $"The pending forms ({ query.Count() }) have been bulk approved by {Context.User.Mention}");
        ...
        public async Task DenyUser(IGuildUser user)
        {
            ...
            await _replyservice.ReplyEmbedAsync(context: Context,
                message: $"User {user.Mention} has been denied verification.");
